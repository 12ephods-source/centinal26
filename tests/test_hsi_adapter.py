...
def test_arbitrary_execution_fields_are_rejected() -> None:
    raw = request(
        "hsi.run",
        {"spec_ref": "x", "spec_sha256": SHA, "command": "rm -rf /"},
    )
...
def test_invalid_hash_fails_closed() -> None:
    with pytest.raises(HSIProtocolError, match="SHA-256"):
        normalize_hsi_request(
            request(parameters={"spec_ref": "x", "spec_sha256": "bad"})
        )
...
def test_runs_are_bounded() -> None:
    with pytest.raises(HSIProtocolError, match="runs"):
        normalize_hsi_request(
            request(
                "hsi.run",
                {"spec_ref": "x", "spec_sha256": SHA, "runs": 1_000_001},
            )
        )


def test_verify_accepts_only_typed_artifact_reference() -> None:
    normalized = normalize_hsi_request(
        request(
            "hsi.verify",
            {
    ...
    assert normalized.parameters["verification_type"] == "evidence_chain"


def test_identify_rejects_run_only_parameters() -> None:
    with pytest.raises(HSIProtocolError, match="run-only"):
        normalize_hsi_request(
            request(
                "hsi.identify",
                {"spec_ref": "x", "spec_sha256": SHA, "runs": 2},
            )
        )


def test_export_format_is_allowlisted() -> None:
    with pytest.raises(HSIProtocolError, match="unsupported export format"):
        normalize_hsi_request(
            request(
                "hsi.export",
                {
