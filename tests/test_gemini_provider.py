...
def test_connected_evidence_promotes_only_to_connected_validated(tmp_path: Path) -> None:
    provider = _provider()
    ...
    record = provider.provider_record(evidence)
    ...
    decision = registry.select(
        "model.generate.text",
        RoutingPolicy(
            minimum_maturity=ProviderMaturity.CONNECTED_VALIDATED,
            allowed_provider_types=("remote_model",),
        ),
    )
    assert decision.provider.provider_id == "gemini-interactions-v1"


def test_failed_probe_keeps_host_maturity_and_marks_unavailable() -> None:
    provider = _provider()
    evidence = GeminiConnectedEvidence(
        passed=False,
        checked_at="2026-08-20T00:00:00+00:00",
...
def test_request_is_stable_v1_store_false_and_tool_choice_none() -> None:
    captured = {}
    ...
    result = _provider(opener).infer("prompt", max_output_tokens=50)
...
def test_receipt_does_not_persist_key_or_plaintext() -> None:
    ...
    result = _provider(opener).infer("prompt body")
...
def test_requires_action_fails_closed() -> None:
    def opener(_request, timeout):
        return FakeResponse(
            {
                "id": "int-1",
                ...
                "steps": [{"type": "function_call", "name": "shell"}],
            }
        )

    with pytest.raises(RuntimeError, match="tool/action execution"):
        _provider(opener).infer("x")
