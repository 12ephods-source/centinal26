...
def test_acquired_transcript_requires_matching_hash_and_preserves_source(tmp_path) -> None:
    store = CanonicalObjectStore(tmp_path / "objects.sqlite3")
    ingestor = ExternalAIEvidenceIngestor(store)
    text = "user: hello\nassistant: hello"
...
def test_partial_source_must_be_explicitly_partial(tmp_path) -> None:
    store = CanonicalObjectStore(tmp_path / "objects.sqlite3")
    ingestor = ExternalAIEvidenceIngestor(store)
...
def test_identical_ingest_is_content_addressed_and_idempotent(tmp_path) -> None:
    store = CanonicalObjectStore(tmp_path / "objects.sqlite3")
    ingestor = ExternalAIEvidenceIngestor(store)
