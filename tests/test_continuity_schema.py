...
def test_generated_schema_matches_single_governed_source() -> None:
    source = continuity_schema.load_source(SOURCE)
    committed = json.loads(SCHEMA.read_text(encoding="utf-8"))
    assert committed == continuity_schema.generate_json_schema(source)


def test_generated_documentation_matches_single_governed_source() -> None:
    source = continuity_schema.load_source(SOURCE)
    assert DOC.read_text(encoding="utf-8") == continuity_schema.generate_markdown(source)
