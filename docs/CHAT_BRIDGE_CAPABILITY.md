...
`conversation.import_chatgpt_export` is a bounded capability for deriving a local, hash-verifiable Markdown conversation corpus from an already preserved ChatGPT provider export. It is not an export acquisition mechanism and it does not grant itself execution authority.
...
The independent verifier re-verifies the provider receipt, manifest hash, path confinement, counts, and every transcript hash. Only after that verification may the runtime reducer update capability state.
