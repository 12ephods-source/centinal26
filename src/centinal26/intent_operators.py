...
# Deliberately small and deterministic. These are stable policy interfaces,
# not a second command language. Unknown prose remains available to planners.
_PHRASES: dict[IntentOperator, tuple[str, ...]] = {
    ...
    IntentOperator.VERIFY: ("verify", "validate", "check it", "test it", "was this actually accomplished"),
    ...
    IntentOperator.COMPRESS: ("combine", "consolidate", "canonicalize", "extract reusable machinery"),
    ...
    for operator, phrases in _PHRASES.items():
        for phrase in phrases:
            if normalized == phrase:
                ...
                # Embedded phrases are useful routing hints but must not silently
                # become high-confidence authorization.
