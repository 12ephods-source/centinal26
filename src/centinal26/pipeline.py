...
class RuntimeStore:
    ...
    def __init__(self, path: Path):
        ...
        self.db.executescript(
            ...
            CREATE TABLE IF NOT EXISTS run_history (
                ...
                state_diverged INTEGER NOT NULL,
    ...
    def _migrate_jobs(self) -> None:
        ...
        if "intent_sha256" not in columns:
            self.db.execute("ALTER TABLE jobs ADD COLUMN intent_sha256 TEXT")
        rows = self.db.execute(
            "SELECT id,intent FROM jobs WHERE intent_sha256 IS NULL OR intent_sha256=''"
        ).fetchall()
        for row in rows:
            intent = json.loads(row["intent"])
            self.db.execute(
                "UPDATE jobs SET intent_sha256=? WHERE id=?",
                (canonical_sha256(intent), row["id"]),
            )
    ...
    def submit(
        ...
        stamp = now_iso()
        intent_json = json.dumps(asdict(intent), sort_keys=True)
        intent_sha256 = intent.sha256
        try:
            self.db.execute(
                """
                INSERT INTO jobs(
                    id,idempotency_key,intent_sha256,intent,grant,capability,state,
                    attempts,max_attempts,lease_until,result,evidence_path,created_at,updated_at
            ...
            self.db.rollback()
            row = self.db.execute(
                "SELECT id,intent_sha256,intent FROM jobs WHERE idempotency_key=?",
                (key,),
            ).fetchone()
            if row is None:
                raise
            existing_sha = row["intent_sha256"] or canonical_sha256(json.loads(row["intent"]))
            if existing_sha != intent_sha256:
                raise IdempotencyConflict(
                    "idempotency key reused with different intent content"
                )
    ...
    def counts(self) -> Json:
        rows = self.db.execute("SELECT state,COUNT(*) n FROM jobs GROUP BY state").fetchall()
        return {row["state"]: row["n"] for row in rows}

    def evolution_status(self, minimum: int = 10) -> Json:
        rows = self.db.execute(
            "SELECT * FROM run_history ORDER BY created_at DESC,run_id DESC"
        ).fetchall()
        consecutive = 0
        for row in rows:
        ...
        recovery_pass = any(row["recovery_test"] and row["verified"] for row in rows)
        verifier_independent = bool(rows) and all(row["verifier_independent"] for row in rows)
        ready = (
            consecutive >= minimum
            and zero_divergence
...
class EvidenceStore:
    ...
    def verify(path: Path) -> bool:
        ...
        return found == canonical_sha256(body)
...
class AutomatedEngine:
    ...
    def submit(
        ...
        try:
            ...
            self.audit.append(
                ...
                {"intent_id": intent.intent_id, "capability": intent.capability, "key": key},
            )
            raise
        self.audit.append(
    ...
    def _execute(self, spec: CapabilitySpec, payload: Json) -> tuple[Json, float]:
        ...
        def timeout_handler(_signum, _frame):
            raise TimeoutError(f"capability exceeded {spec.timeout_seconds:.3f}s bound")
    ...
    def run_once(self, recovery_test: bool = False) -> str | None:
        ...
        except Exception as error:  # noqa: BLE001 - independent verifier boundary
            verified = False
            verifier_error = {"error": type(error).__name__, "message": str(error)}
            ...
            try:
                ...
                if observed != updated:
                    ...
                    divergence_path = self.evidence.write(
                        ...
                        attempt,
                        "state-divergence",
                        {"output_evidence": str(evidence_path), "state_divergence": result},
                    )
                    self.store.finish(
                        job_id,
                        "state_diverged",
                    ...
                    self.audit.append("job_state_diverged", {"job_id": job_id, **result})
                    self.store.record(
                        False,
                    ...
                    return job_id
            except Exception as error:  # noqa: BLE001 - do not replay verified effects
                result = {
                    "ok": False,
                    "verified": True,
                ...
                self.audit.append("job_state_update_failed", {"job_id": job_id, **result})
                self.store.record(
                    False,
        ...
        self.store.record(True, True, recovery_test, spec.verifier_independent)
        return job_id
