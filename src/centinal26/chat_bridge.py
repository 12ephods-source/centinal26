...
def _node_path_to_current(conversation: Json) -> list[Json]:
    """Return the selected ChatGPT branch instead of merging sibling branches."""
...
def _messages_from_conversation(conversation: Json) -> list[Json]:
    ...
    for node in nodes:
        ...
        messages.append({"role": role, "text": text, "create_time": message.get("create_time"), "message_id": message.get("id")})
    ...
    try:
        if export_path.suffix.lower() == ".zip" or zipfile.is_zipfile(export_path):
            with zipfile.ZipFile(export_path) as archive:
                candidates = sorted((info for info in archive.infolist() if not info.is_dir() and Path(info.filename).name.lower() == "conversations.json"), key=lambda info: (len(Path(info.filename).parts), info.filename))
                if not candidates:
                    raise ChatBridgeError("The ZIP does not contain conversations.json.")
                with archive.open(candidates[0], "r") as raw, io.TextIOWrapper(raw, encoding="utf-8-sig") as stream:
...
def import_chatgpt_export(export_path: Path, destination: Path, *, source_receipt_id: str | None = None) -> Json:
    ...
    for number, conversation in enumerate(_iter_chatgpt_conversations(data), start=1):
        ...
        conversation_id = str(conversation.get("id") or conversation.get("conversation_id") or number)
        messages = _messages_from_conversation(conversation)
        total_messages += len(messages)
        md_path = destination / f"{number:05d}-{_safe_filename_component(title)}.md"
        lines = [f"# {_display_title(title)}", "", f"Conversation ID: `{conversation_id}`", ""]
        for item in messages:
            lines.extend([f"## {item['role'].title()}", "", item["text"], ""])
        markdown = "\n".join(lines).rstrip() + "\n"
        _atomic_write_text(md_path, markdown)
        index.append({"conversation_id": conversation_id, "title": title, "messages": len(messages), "relative_path": md_path.name, "sha256": _sha256_text(markdown)})
    manifest: Json = {"schema_version": 1, "imported_at": _utc_now_iso(), "source_receipt_id": source_receipt_id, "source_size_bytes": export_path.stat().st_size, "source_sha256": _sha256_file(export_path), "conversation_count": len(index), "message_count": total_messages, "conversations": index}
...
def import_verified_chatgpt_export(payload: Json, *, home: Path, evidence_root: Path = DEFAULT_ROOT) -> Json:
    ...
    manifest = import_chatgpt_export(Path(verified.object_path), destination, source_receipt_id=receipt_id)
    manifest_path = destination / "chatgpt-import-manifest.json"
    return {"receipt_id": receipt_id, "source_sha256": verified.sha256, "source_size_bytes": verified.size, "destination": str(destination), "manifest_path": str(manifest_path), "manifest_sha256": _sha256_file(manifest_path), "conversation_count": manifest["conversation_count"], "message_count": manifest["message_count"]}


def verify_chatgpt_import(payload: Json, output: Json, *, home: Path, evidence_root: Path = DEFAULT_ROOT) -> bool:
    try:
        receipt_id = _required_receipt_id(payload)
        verified = verify_receipt(evidence_root, receipt_id)
        destination = _import_destination(home, receipt_id)
        manifest_path = destination / "chatgpt-import-manifest.json"
        if output.get("receipt_id") != receipt_id or output.get("source_sha256") != verified.sha256 or output.get("source_size_bytes") != verified.size:
            return False
        if Path(str(output.get("destination", ""))).resolve() != destination or Path(str(output.get("manifest_path", ""))).resolve() != manifest_path:
            return False
        if not manifest_path.is_file() or output.get("manifest_sha256") != _sha256_file(manifest_path):
            return False
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        conversations = manifest.get("conversations")
        if not isinstance(conversations, list) or manifest.get("schema_version") != 1 or manifest.get("source_receipt_id") != receipt_id or manifest.get("source_sha256") != verified.sha256 or manifest.get("source_size_bytes") != verified.size:
            return False
        if manifest.get("conversation_count") != len(conversations) or output.get("conversation_count") != len(conversations):
            return False
        total_messages = 0
        ...
        return manifest.get("message_count") == total_messages and output.get("message_count") == total_messages
...
def chat_import_reducer(current: Json, output: Json) -> Json:
    return {"verified_runs": int(current.get("verified_runs", 0)) + 1, "last_receipt_id": output["receipt_id"], "last_source_sha256": output["source_sha256"], "last_manifest_sha256": output["manifest_sha256"], "last_conversation_count": output["conversation_count"], "last_message_count": output["message_count"]}


def chat_export_import_capability(home: Path, *, evidence_root: Path | None = None) -> CapabilitySpec:
    ...
    def execute(payload: Json) -> Json:
        return import_verified_chatgpt_export(payload, home=resolved_home, evidence_root=resolved_evidence_root)
    def verify(payload: Json, output: Json) -> bool:
        return verify_chatgpt_import(payload, output, home=resolved_home, evidence_root=resolved_evidence_root)
    return CapabilitySpec(name="conversation.import_chatgpt_export", executor=execute, verifier=verify, reducer=chat_import_reducer, timeout_seconds=120.0, max_attempts=2, verifier_independent=True)


def register_chat_bridge_capabilities(engine: AutomatedEngine, home: Path, *, evidence_root: Path | None = None) -> None:
