...
_FORBIDDEN_SECRET_KEYS = frozenset(
    {"password", "secret", "token_value", "api_key_value", "credential_value", "private_key"}
)
...
def validate_manifest_dict(
    ...
    *,
    ...
    if manifest.schema_version != "wazoo26.adapter/0.2":
    ...
    expected_version = {
        ProtocolFamily.A2A: A2A_VERSION,
        ProtocolFamily.MCP: MCP_VERSION,
    }[manifest.protocol_family]
    if manifest.protocol_version != expected_version:
        errors.append(
            f"{manifest.protocol_family.value} protocol must be pinned to {expected_version}"
        )
    ...
    try:
        ...
        errors.append("adapter is absent from the Wazoo26 federation catalog")
        descriptor = None
    ...
    if manifest.protocol_family is ProtocolFamily.MCP:
    ...
    for capability in manifest.capabilities:
        ...
        if capability.side_effects == "irreversible" and capability.verification != "required":
            errors.append(
                f"{capability.capability_id}: irreversible capability requires verification"
            )
        if capability.timeout_ms <= 0:
            errors.append(f"{capability.capability_id}: timeout_ms must be positive")
        if capability.concurrency_limit <= 0:
            errors.append(f"{capability.capability_id}: concurrency_limit must be positive")
        ...
        if descriptor is not None and capability.canonical_operation not in descriptor.operations:
            errors.append(
                f"{capability.capability_id}: canonical operation is not registered "
                f"for {manifest.adapter_id}"
            )
