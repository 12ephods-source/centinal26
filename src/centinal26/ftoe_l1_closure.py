...
def operator_scan(inputs: L1Inputs, n_min: int = 1, n_max: int = 12) -> list[OperatorCandidate]:
    ...
    return [OperatorCandidate(n, hierarchy / ratio**n) for n in range(n_min, n_max + 1)]
...
def evaluate_l1(
    ...
    *,
    ...
    """Evaluate the frozen L1 compatibility gates without promoting open UV gates."""
    if inputs is None:
        inputs = L1Inputs()
