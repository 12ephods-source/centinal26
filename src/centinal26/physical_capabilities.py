...
class BoundedLocalExecutor:
    ...
    def run(self, name: str) -> Json:
        ...
        completed = subprocess.run(
            spec.argv, check=False, capture_output=True, text=True,
            timeout=spec.timeout_s, shell=False,
        )
        return {
            "operation": name,
            "argv": list(spec.argv),
...
def _verify_sha256(payload: Json, output: Json) -> bool:
    ...
    if not path.is_file():
        return False
    return output.get("sha256") == hashlib.sha256(path.read_bytes()).hexdigest()
...
def register_physical_capabilities(
    ...
    *,
    ...
    """Register minimal read-only device capabilities; no arbitrary shell."""
    ...
    registry.register("device.python_version", lambda payload: local.run("device.python_version"), _verify_zero_exit)
    registry.register("device.uname", lambda payload: local.run("device.uname"), _verify_zero_exit)
