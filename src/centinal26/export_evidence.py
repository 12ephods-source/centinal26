...
def _strong_identity_conflict(
    existing: dict[str, Any], *, provider: str, message_id: str | None, export_id: str | None, sha256: str
) -> bool:
...
def _preserve_object(source: Path, target: Path, expected_sha256: str, expected_size: int) -> bool:
    if target.exists():
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preserve provider export bytes in the neutral Centinal26 evidence store."
    )
