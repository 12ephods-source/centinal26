...
class ExecutionPolicy:
    # Exact EXECUTE language is authorization to advance one already-bounded
    # operation. It is never authority to invent a capability or bypass gates.
...
class IntentExecutionController:
    """Bounded vertical slice: ingress -> authorization -> execution -> evidence -> reconciliation.

    The controller intentionally supports EXECUTE first. Other intent operators are
    routing interfaces and must receive operator-specific policies before execution.
    """

    def __init__(self, store: EventStore, registry: CapabilityRegistry,
    ...
    def ingest_and_execute(self, *, text: str, adapter_id: str, external_id: str,
                           capability: str, payload: Json,
                           constraints: Json | None = None,
                           actor: str | None = None) -> VerticalExecutionResult:
        match = classify_intent(text)
        self._require_exact_execute(match)

        ingest = self.gateway.ingest(AdapterRequest(
        ...
        self.store.append("TASK_AUTHORIZED", {
            "task_id": task_id,
        ...
        self.store.append("TASK_STARTED", {
            "task_id": task_id,
            "capability": capability,
        }, entity_id=task_id)
        ...
        self.store.append("TASK_EXECUTED", {
            **evidence,
            "evidence_digest": evidence_digest,
        }, entity_id=task_id)

        passed = bool(verify(payload, output))
        if not passed:
            self.store.append("VERIFICATION_FAILED", {
                "task_id": task_id,
                "evidence_digest": evidence_digest,
            }, entity_id=task_id)
            raise IntentExecutionError("independent verification failed")

        self.store.append("VERIFICATION_PASSED", {
            "task_id": task_id,
            "evidence_digest": evidence_digest,
        }, entity_id=task_id)
        self.store.append("TASK_COMPLETED", {
            "task_id": task_id,
            "evidence_digest": evidence_digest,
        }, entity_id=task_id)

        final_state = rebuild_state(self.store.events())
        completed = final_state.tasks[task_id].get("status") == "COMPLETE"
