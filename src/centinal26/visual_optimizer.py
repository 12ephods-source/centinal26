...
Provider-neutral orchestration for iterative image improvement. Rejected
candidates never become parents; providers supply generation and evaluation.
"""
...
DEFAULT_DRAGONIA_QUEUE = (
    Defect("white_dragon_cardinality", "Upper-right only: consolidate duplicate white-dragon heads into exactly one coherent white dragon. Add nothing else."),
    ...
    Defect("reidrick_attack_edge", "Correct Reidrick attack only: blue-white lightning originates inside his mouth and terminates on the red dragon."),
    Defect("grumm_attack_edge", "Correct Grumm attack only: one narrow green beam originates at the central pupil and terminates on the single white dragon."),
    Defect("cumulus_scale", "Reduce and distance Cumulus only; preserve foreground and castle composition."),
    Defect("combat_cues", "Add only minimal combat cues to locked foreground poses; do not reposition or redesign characters."),
    Defect("chariot_motion", "Improve only upward/toward-viewer motion using subtle foreshortening, taut harness lines and directional atmosphere."),
...
class VisualOptimizer:
    ...
    def optimize(
        self,
        canonical_artifact: str,
        defects: Sequence[Defect] = DEFAULT_DRAGONIA_QUEUE,
        locks: Sequence[str] = DRAGONIA_LOCKS,
    ) -> str:
        """Run bounded coordinate descent with rollback on rejected candidates."""
        current = canonical_artifact
        previous_hash = "GENESIS"
        for iteration, defect in enumerate(defects, start=1):
            if iteration > self.policy.max_iterations:
                break
            candidate_ref = self.provider.generate(
                canonical_artifact=current, defect=defect, locks=locks
            )
