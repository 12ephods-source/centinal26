...
class GeminiProviderConfig:
    ...
    allowed_models: tuple[str, ...] = STABLE_MODELS
    ...
    def validate(self) -> None:
        ...
        if not self.allowed_models or any(model not in STABLE_MODELS for model in self.allowed_models):
            raise ValueError("only stable approved Gemini models may be allowlisted")
        if any(p not in (PrivacyClass.PUBLIC, PrivacyClass.INTERNAL) for p in self.allowed_privacy):
...
class GeminiInteractionsProvider:
    """Bounded Gemini text provider for the Frost provider registry.

    This class is an execution adapter only. It never verifies the semantic truth
    of a model response and never promotes model output into canonical state.
    """
    ...
    def provider_record(
        self,
        evidence: GeminiConnectedEvidence | None = None,
    ) -> ProviderRecord:
    ...
    def infer(
        ...
        *,
        ...
        if status == "requires_action":
            raise RuntimeError("Gemini requested tool/action execution; provider is fail-closed")
    ...
    def connected_probe(self) -> GeminiConnectedEvidence:
        ...
        try:
            ...
            passed = result.status == "completed" and result.text.strip() == "GEMINI_OK"
