...
TMP_ROOT="${TMPDIR:-$PREFIX/tmp}/centinal26-fleet-v1.5.$$"

say(){ printf '[frost-fleet-v1.5] %s\n' "$*"; }
die(){ printf '[frost-fleet-v1.5] ERROR: %s\n' "$*" >&2; exit 1; }
...
fetch_verify_run(){
  local commit="$1" path="$2" expected_blob="$3" out="$TMP_ROOT/$(basename "$path")"
...
FROST FLEET BOOTSTRAP v1.5 COMPLETE
