    ...
    try:
        for index, capability in enumerate(("device.python_version", "device.uname"), start=1):
            result = controller.ingest_and_execute(
                text="Proceed",
                adapter_id="termux-physical-gate",
                external_id=f"{run_id}-{index}",
        ...
        report = {
            "schema": "centinal26.physical_intent_gate.v1",
            "run_id": run_id,
            "platform": platform.platform(),
            "python": sys.version,
            "results": results,
            "event_chain_valid": store.verify_chain(),
            "status": "PASS" if all(item["completed"] and item["verified"] for item in results) else "FAIL",
        }
        path = evidence_dir / f"PHYSICAL_INTENT_GATE_{run_id}.json"
        path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print(path)
        print(report["status"])
        return 0 if report["status"] == "PASS" and report["event_chain_valid"] else 1
    finally:
        store.close()
