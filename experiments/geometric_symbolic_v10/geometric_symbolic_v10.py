"""Paired-counterfactual neural grounding into an exact latent algebra.

Successor to v9. No token-level latent labels are supplied. Every training step
contains a canonical noisy surface sequence and a perturbation in which one
load-bearing latent element is incremented by one modulo four. Both final
sequence labels are supplied, plus the same output-level counterfactual
consistency loss for every model family.

This tests whether paired perturbation supervision resolves the neural grounding
failure observed in v9. It is synthetic and not a Sophontic reproduction.
"""
...
def main():
    ...
    for mode in MODES:
        ...
        for metric in ("train_pair", "near_acc", "near_pair", "far_acc", "far_pair"):
    ...
    payload = {
        "experiment": {
            "successor_to": "PR #154 v9 neural-grounding FAIL",
            "claim": "paired counterfactual supervision can ground neural surfaces into exact algebra",
            "fresh_seeds": list(SEEDS),
            "train_curriculum": "paired lengths 2->5",
            "near_lengths": list(NEAR_LENGTHS),
            "far_lengths": list(FAR_LENGTHS),
            "supervision": "canonical and perturbed final sequence labels only",
