...
def start_daemon() -> None:
    subprocess.run(
        [str(BIN / "centinal26_daemon_service.sh"), "start"],
        check=False,
...
def cycle() -> None:
    cp = subprocess.run(
        ["python", str(BIN / "centinal26_improvement_cycle.py")],
        text=True,
        capture_output=True,
        timeout=120,
        check=False,
    ...
    with (LOGS / "guardian.jsonl").open("a") as handle:
