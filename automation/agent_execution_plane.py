...
def run_task(task, root: Path):
    ...
    result = {"status":"PASS" if proc.returncode == 0 else "FAIL","role":role,"returncode":proc.returncode,
              "stdout":proc.stdout[-12000:],"stderr":proc.stderr[-12000:],"elapsed_s":round(time.time()-started,3),
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("task"); p.add_argument("--root",default="."); p.add_argument("--evidence",default="agent_evidence.json")
    a=p.parse_args(); task=json.loads(Path(a.task).read_text()); result=run_task(task,Path(a.root)); Path(a.evidence).write_text(json.dumps(result,indent=2)+"\n"); print(json.dumps(result,indent=2)); raise SystemExit(0 if result["status"]=="PASS" else 1)
