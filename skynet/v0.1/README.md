...
`project_update` is a paired transaction: both configured repositories are preflighted before mutation, only clean fast-forward promotions are allowed, and earlier promotions are rolled back when a later promotion fails and the rollback remains safe.
