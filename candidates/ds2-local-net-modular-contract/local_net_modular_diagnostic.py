...
BETA = 2.0 * math.pi
LATTICE_SIZES = (16, 32, 64, 128, 256, 512, 1024)
...
def smearing_specs() -> tuple[dict[str, object], ...]:
    return (
        {"name": "A_left", "region": (0.18, 0.42), "center": 0.29, "radius": 0.075},
        {"name": "A_right", "region": (0.18, 0.42), "center": 0.35, "radius": 0.050},
        {"name": "B_middle", "region": (0.12, 0.62), "center": 0.48, "radius": 0.090},
        {"name": "C_outer", "region": (0.05, 0.95), "center": 0.67, "radius": 0.100},
    )
...
def continuum_smearings(x: np.ndarray) -> dict[str, dict[str, object]]:
    ...
    for spec in smearing_specs():
        raw = compact_bump(x, float(spec["center"]), float(spec["radius"]))
        q = l2_normalize(raw, x)
        out[str(spec["name"])] = {"region": tuple(spec["region"]), "q": q}
    return out


def sine_coefficients(x: np.ndarray, values: np.ndarray, max_mode: int) -> np.ndarray:
...
def continuum_vectors(max_mode: int) -> dict[str, tuple[np.ndarray, np.ndarray]]:
    ...
    for name, data in continuum_smearings(x).items():
        f = sine_coefficients(x, data["q"], max_mode)
        vectors[name] = (f, np.zeros_like(f))
    return vectors


def dst1_normalized(values: np.ndarray) -> np.ndarray:
    """Return U^T values for U_jk=sqrt(2/(N+1)) sin(pi*j*k/(N+1))."""
    n = values.size
    extended = np.zeros(2 * (n + 1), dtype=float)
    extended[1 : n + 1] = values
    extended[n + 2 :] = -values[::-1]
    sine_sums = -0.5 * np.fft.fft(extended).imag[1 : n + 1]
    return math.sqrt(2.0 / (n + 1)) * sine_sums


def lattice_vectors(size: int) -> dict[str, tuple[np.ndarray, np.ndarray]]:
    dx = 1.0 / (size + 1)
    x = np.arange(1, size + 1, dtype=float) * dx
    vectors: dict[str, tuple[np.ndarray, np.ndarray]] = {}
    for spec in smearing_specs():
        raw = compact_bump(x, float(spec["center"]), float(spec["radius"]))
        norm = math.sqrt(dx * float(np.sum(raw * raw)))
        if norm <= 0.0:
            raise ValueError("lattice failed to resolve compact smearing")
        sampled = raw / norm
        f = math.sqrt(dx) * dst1_normalized(sampled)
        vectors[str(spec["name"])] = (f, np.zeros_like(f))
    return vectors
...
def lattice_omegas(size: int) -> np.ndarray:
    ...
    return (2.0 / dx) * np.sin(math.pi * mode_numbers / (2.0 * (size + 1)))
...
def modular_evolve(
    ...
    # rho_beta proportional to exp(-beta H) implies sigma_s=alpha_{-beta*s}.
    ...
    for left_name, left in vectors.items():
        for right_name, right in vectors.items():
            for s in MODULAR_S:
                ...
                before = covariance(right, right, omegas)
                after = covariance(evolved, evolved, omegas)
                invariance = abs(before - after) / max(1.0, abs(before))
...
def locality_checks() -> dict[str, object]:
    ...
    regions = {"A": (0.18, 0.42), "B": (0.12, 0.62), "C": (0.05, 0.95)}
    for name, data in smearings.items():
        region = tuple(data["region"])
        q = data["q"]
        outside = (x <= region[0]) | (x >= region[1])
        support_residual = max(support_residual, float(np.max(np.abs(q[outside]))))
        valid_regions = []
...
def wrong_modular_direction_gap() -> float:
    ...
    wrong = weyl_two_point(left, evolve_for_time(right, omegas, +BETA * s), omegas)
    return float(abs(correct - wrong))


def evaluate() -> dict[str, object]:
    continuum_results = {
        max_mode: diagnostic_for(continuum_vectors(max_mode), continuum_omegas(max_mode))
    ...
    fit_sizes = LATTICE_SIZES[-5:]
    fit_dx = np.array([1.0 / (size + 1) for size in fit_sizes])
    fit_errors = np.array(errors[-5:])
    ...
    return {
        "schema": "ds2.local-net-modular-diagnostic.v2",
        "execution_pass": passed,
        "scientific_pass": False,
        "status": "PASS_FROZEN_FIELD_WEYL_MODULAR_CORRELATOR_SUBGATE"
        if passed
        else "FAIL_FROZEN_FIELD_WEYL_MODULAR_CORRELATOR_SUBGATE",
        "checks": checks,
        "continuum_reference": {
        ...
        "lattice_refinement": {
            ...
            "rows": lattice_rows,
            "errors": errors,
            "observed_order_last_five": observed_order,
        },
        "negative_control": {
        ...
        "interpretation": {
            "established_if_pass": [
                "controlled finite-to-continuum convergence on one frozen finite family of compactly supported field smearings",
                "convergence of complex thermal modular-flow Weyl two-point functions on the frozen modular-parameter grid",
                "modular evolution generates the conjugate momentum component from the initial field-smeared Weyl probes",
                "consistent nested support bookkeeping for the frozen A subset B subset C interval family",
                "the negative control rejects reversal of the thermal modular-flow direction",
            ],
            "not_established": [
                "initial momentum-smeared or arbitrary phase-space Weyl convergence",
            ...
            "promotion_ceiling": "FROZEN_FIELD_TEST_FAMILY_NECESSARY_SUBGATE_ONLY",
