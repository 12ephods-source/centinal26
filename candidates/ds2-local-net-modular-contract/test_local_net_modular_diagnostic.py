...
def test_modular_evolution_generates_momentum_component() -> None:
    vectors = diagnostic.continuum_vectors(128)
    omegas = diagnostic.continuum_omegas(128)
    evolved = diagnostic.modular_evolve(vectors["A_left"], omegas, 0.1)
    assert float(abs(evolved[1]).max()) > 1.0e-6
...
def test_diagnostic_passes_only_frozen_field_family_subgate() -> None:
    result = diagnostic.evaluate()
    assert result["execution_pass"] is True
    assert result["scientific_pass"] is False
    assert result["status"] == "PASS_FROZEN_FIELD_WEYL_MODULAR_CORRELATOR_SUBGATE"
    assert result["negative_control"]["detected"] is True
    interpretation = result["interpretation"]
    assert interpretation["promotion_ceiling"] == (
        "FROZEN_FIELD_TEST_FAMILY_NECESSARY_SUBGATE_ONLY"
    )
    assert interpretation["next_gate"] == (
        "COMMON_GNS_OR_STANDARD_SUBSPACE_OPERATOR_TOPOLOGY_CONVERGENCE"
    )
    assert "initial momentum-smeared or arbitrary phase-space Weyl convergence" in interpretation[
        "not_established"
    ]
...
def test_refinement_is_material_not_single_cutoff_coincidence() -> None:
    ...
    assert refinement["observed_order_last_five"] >= 1.5
