...
# Matrix ordering: (3,2,1) for G321 and (4,2L,2R) for G422.
...
def low_energy_couplings(mu: float, threshold: float = M_I_PHYS) -> Dict[str, float]:
    ...
    at = {k: evolve_alpha_inv(start[k], B_SM[k], MZ, threshold) for k in start}
    return {k: evolve_alpha_inv(at[k], B_2H[k], threshold, mu) for k in start}
...
def find_log_sign_change(fn, lo: float, hi: float, samples: int = 80) -> Tuple[float, float]:
...
def solve_422_unification(mi: float) -> Tuple[float, float, Dict[str, float]]:
    ...
    log_ratio = 2.0*PI*(aL-a4)/(B_422["L"]-B_422["4"])
    ...
    avg = (au4+auL+auR)/3.0
    return mu, 1.0/avg, {"4": au4, "L": auL, "R": auR}


def _rhs_two_loop(inv: Sequence[float], a: Sequence[float], bij: Sequence[Sequence[float]]) -> Tuple[float, ...]:
    alpha = [1.0/x for x in inv]
    return tuple(
        -a[i]/(2.0*PI) - sum(bij[i][j]*alpha[j] for j in range(len(inv)))/(8.0*PI*PI)
...
def _rk4_step(inv: Sequence[float], h: float, a: Sequence[float], bij: Sequence[Sequence[float]]) -> Tuple[float, ...]:
    ...
    return tuple(y + h*(p+2*q+2*r+s)/6.0 for y, p, q, r, s in zip(inv, k1, k2, k3, k4))
...
def low_energy_couplings_two_loop(mu: float, threshold: float = M_I_PHYS) -> Dict[str, float]:
    if mu < MZ:
        raise ValueError("mu must be >= MZ")
    ...
    if threshold <= MZ:
        ...
        yt = evolve_two_loop(y0, MZ, threshold, A_SM_321, BIJ_SM_321)
        y = evolve_two_loop(yt, threshold, mu, A_2HDM_321, BIJ_2HDM_321)
...
def shoot_422_two_loop(mi: float, threshold: float = M_I_PHYS, step_log: float = 0.006, max_mu: float = 1e19) -> Tuple[float, float, Dict[str, float]]:
    ...
    while t < tmax:
        ...
        if prev_f == 0.0 or prev_f*f <= 0.0:
            ...
            return math.exp(tc), yc[2]-yc[0], {"4": yc[0], "L": yc[1], "R": yc[2]}
...
def solve_two_loop_422(threshold: float = M_I_PHYS) -> Tuple[float, float, float, Dict[str, float], float]:
    """Solve gauge-only two-loop M_I and M_U with configurable second-doublet threshold."""
    fn = lambda x: two_loop_mi_residual(x, threshold=threshold)
    lo, hi = find_log_sign_change(fn, 1e6, 1e14, samples=100)
    mi = lo if lo == hi else bisect_log_root(fn, lo, hi, iterations=80, tol=2e-5)
    mu, _residual, inv = shoot_422_two_loop(mi, threshold=threshold, step_log=0.0025)
...
def calculate() -> Result:
    ...
    # First reproduce the published zero-threshold 2HDM calculation class as a
    # regression check, then run the FToE-specific 13.5 TeV threshold.
    ref_mi, ref_mu, _ref_alpha, _ref_inv, ref_spread = solve_two_loop_422(threshold=MZ)
    ...
    if any(gates[g] == "FAIL" for g in mandatory):
        status = "FAIL"
    elif any(gates[g] in {"NOT_TESTED", "REVIEW"} for g in mandatory):
        status = "REVIEW"
    else:
        status = "PASS"
